...
public class HotelController extends BaseController implements Initializable {
	...
	@FXML
	void cancelarHotel(ActionEvent event) { //OJITOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
	...
	void nuevoHotel(ActionEvent event) {
		if (this.verificar()) {
			int indice = Hoteles.getInstancia().getGrupoHoteles().size() - 1;
			Hotel hotel = Hoteles.getInstancia().getGrupoHoteles().get(indice);
			...
			if (Hoteles.getInstancia().isModificarHotel()) {
				...
				Hoteles.getInstancia().getGrupoHoteles().set(indice, hotel);
			}
			this.cerrarVentana(btnAceptar);
		}else {
			//System.out.println("mensajes "+this.mensajes);
	...
	private boolean verificar() {
		...
		if ((this.txtDireccion.getText() == null)|| (this.txtDireccion.getText() != null && this.txtDireccion.getText().isEmpty())) {
			this.mensajes += "La direccion del hotel no es valido, es vacio\n";
			valido = false;
		}
		if ((this.txtCorreo.getText() == null)|| (this.txtCorreo.getText() != null && this.txtCorreo.getText().isEmpty())) {
			this.mensajes += "El correo del hotel no es valido, es vacio\n";
			valido = false;
		}
		if (!(this.txtCorreo.getText() == null)|| (this.txtCorreo.getText() != null && this.txtCorreo.getText().isEmpty())) {
		...
		if ((this.txtTelefono.getText() == null)|| (this.txtTelefono.getText() != null && this.txtTelefono.getText().isEmpty())) {
			this.mensajes += "El telefono del hotel no es valido, es vacio\n";
			valido = false;
		}
		if (!(this.txtTelefono.getText() == null)|| (this.txtTelefono.getText() != null && this.txtTelefono.getText().isEmpty())) {
			if (!this.telefonoValido) {
				this.mensajes += "El telefono del hotel no es valido, minimo=10, maximo=10\n";
				valido = false;
			}
		}
		return valido;
